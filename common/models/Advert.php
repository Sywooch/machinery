...
class Advert extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['created', 'updated', 'published', 'order_options', 'manufacture', 'color'], 'safe'],
...
//        $this->owner->category = $categories;
        if(is_string($this->owner->manufacture[0])){
            if(!$manufacture = TaxonomyItems::find()->where(['lower(name)' => strtolower($this->owner->manufacture[0])])->andWhere(['vid'=>self::VCL_MANUFACTURES])->one()){
                $manufacture = new TaxonomyItems();
                $manufacture->name = $this->owner->manufacture[0];
                $manufacture->vid = self::VCL_MANUFACTURES;
                $manufacture->save();
            }
            $this->owner->manufacture = [];
            $this->owner->manufacture[$manufacture->id] = $manufacture;
        }
//        dd($this->owner->category);
//        dd($this->owner->manufacture);
