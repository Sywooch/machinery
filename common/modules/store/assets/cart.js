...
buyButton.addUrl = '/store/cart/add';
buyButton.init = function(){
   $('.'+this.btnClass).click(this.click);
}
buyButton.click = function(){
    var data = {entityId: $(this).attr('entityId'), model: $(this).attr('entity')};    
    $.ajax({
...
buyButton.confirmModal = function(product){
    modal.show('Вы добавили в корзину',this.product2TextFormater(product));
}
...
cart.timeoutId = 0;
cart.countUrl = '/store/cart/count';
cart.init = function(){
    if(!cart.page.length){
        return;
    }
